...
\begin{definition}
	Sea $x=x_1\cdots x_n\in\Gamma^n$, definimos el \define{soporte}{soporte} de $x$ como $sop(x)=\set{i\tq x_i\neq 0, i=1,\dots, n}$.
\end{definition}

\begin{definition}
	Sea $x\in\Gamma^n$, definimos el \define{peso}{q-peso} de $x$ como $w(x)=|sop(x)|$.
\end{definition}
\begin{lemma}
...
\begin{definition}
	Sea $C$ un código, definimos el \define{peso}{peso-codigo} de $C$ a $w(C)=\min_{x\in C}\set{w(x)\tq x\neq 0}$.
\end{definition}
